#import <Foundation/Foundation.h>
#include <substrate.h>
#include <mach-o/dyld.h>

#define LOG(fmt, ...) NSLog(@"[ZeroTieriOSFix] " fmt "\n", ##__VA_ARGS__)

// The code below should be replaced with your own planet server data
#define ZT_DEFAULT_WORLD_LENGTH 551
static const unsigned char ZT_DEFAULT_WORLD[ZT_DEFAULT_WORLD_LENGTH] = {};

// static int64_t (*old_sub_3806C)(int64_t a1, char *a2, unsigned int a3);
// static int64_t new_sub_3806C(int64_t a1, char *a2, unsigned int a3) {
// 	LOG(@"sub_3806C");
// 	memcpy(a2, (char *)ZT_DEFAULT_WORLD, ZT_DEFAULT_WORLD_LENGTH);
// 	return old_sub_3806C(a1, a2, a3);
// }

static int64_t (*old_sub_378DC)(int64_t a1, char *a2, unsigned int a3);
static int64_t new_sub_378DC(int64_t a1, char *a2, unsigned int a3) {
	LOG(@"sub_378DC");
	memcpy(a2, (char *)ZT_DEFAULT_WORLD, ZT_DEFAULT_WORLD_LENGTH);
	return old_sub_378DC(a1, a2, a3);
}


static bool findImageSlide(const char *name, intptr_t *slide){
    for (uint32_t i = 0; _dyld_image_count(); i++) {
        const char *path = _dyld_get_image_name(i);
        if (strstr(path, name)) {
            *slide = _dyld_get_image_vmaddr_slide(i);
            return true;
        }
    }
    return false;
}

%ctor {
  LOG(@"loaded in %s (%d)", getprogname(), getpid());

  // The code only supports the 1.8.10 version of the iOS client
  // You should replace the planet server above with your own
  intptr_t addr = 0;
  if(findImageSlide("ZeroTier.framework/ZeroTier", &addr)){
	LOG(@"hook sub_378DC at %p", (void*)addr);
	// MSHookFunction(
	// 	(void *)(addr + 0x3806C),
	// 	(void *)new_sub_3806C,
	// 	(void **)&old_sub_3806C
	// );
	MSHookFunction(
		(void *)(addr + 0x378DC), // This is an offset from version 1.8.10, other versions may not apply
		(void *)new_sub_378DC,
		(void **)&old_sub_378DC
	);
  }
}